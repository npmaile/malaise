...
| [`mjit/`](mjit/) | C99 | A bytecode VM and loop JIT for a restricted, syntactically-incompatible dialect of Malaise. Real x86-64 machine code, `mmap`'d and executed, for exactly one loop shape. Everything else interprets forever. |
...
To use the ecosystem you install all ~17 language runtimes (one of them
isn't a runtime at all, just a kernel), at least three of which are
mutually exclusive by operating system, exactly one of which runs on all
three anyway, and one of which will, under narrow conditions, write and
execute its own machine code at you. See
